...
export default function ThrCalculator() {
  ...
  const text = `Dengan upah ${rupiah.format(wage)} dan masa kerja ${months} bulan, estimasi THR saya di KerjaTools adalah ${rupiah.format(result)}.`;
  ...
  return <div className="tool-grid">
    ...
    <div className="result"><small>Estimasi THR</small><strong>{rupiah.format(result)}</strong><p>Dengan upah {rupiah.format(wage)} dan masa kerja {months} bulan.</p><ResultActions text={text} /></div>
  </div>;
}
